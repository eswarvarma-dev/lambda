// Implementation shamelessly stolen from Angular 2
library lambda.zone;
 ...
 * in the "inner" zone unless `runOutsideLambda` is explicitely called.
 *
 * A typical application will create a singleton `NgZone`. The mount zone is the `Zone` where the singleton has been
 * instantiated. The default `onTurnDone` runs the Lambda change detection.
 */
class NgZone {
  ...
  /**
   * Sets the zone hook that is called just before Lambda event turn starts.
  ...
  /**
   * Sets the zone hook that is called immediately after Lambda processes
   ...
   * an event completes. At this point Lambda will no longer attempt to
   * sync the UI. Any changes to the data model will not be reflected in the
   * DOM. `onEventDoneFn` is executed outside Lambda zone.
   *
   ...
   * Lambda zone. The first argument is the error. The second argument is
   * the stack trace.
   */
   ...
   *
   * In a typical app where the inner zone is the Lambda zone, this allows one to make use of the
   * Lambda's auto digest mechanism.
   *
   * ```
   ...
   *
   * In a typical app where the inner zone is the Lambda zone, this allows one to escape Lambda's
